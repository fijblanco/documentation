...
Add these fields in Odoo by navigating to (in :ref:`developer mode <developer-mode>`)
:menuselection:`Settings --> Technical Menu --> Parameters --> System Parameters`. Change the value
of the parameter so that it fits the company's needs.
...
this alias is allowed to create in the give period of time (value of `mail.incoming.limit.period`).
...
received match that of the email being received during that time-frame and prevent a feedback loop
from occurring if a duplicate email should be present.

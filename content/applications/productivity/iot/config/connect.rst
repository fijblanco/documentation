...
the :abbr:`IoT (Internet of Things)` box into to a power source.
...
The following is the process to connect the :abbr:`IoT (Internet of Things)` box via an WiFi
connection to the Odoo database.

First, ensure there is no ethernet cable plugged into the :abbr:`IoT (Internet of Things)` box.
Then, connect all wired devices to the :abbr:`IoT (Internet of Things)` box (ethernet, :abbr:`USB
(Universal Serial Bus)` devices, etc.).
...
Things)` box, then paste the previously copied *token* into the :guilabel:`Server Token` field, and
then click on :guilabel:`Next`.
...
Things)` app can be made using the *token* from a computer, which can be found by navigating to
...
appears, click :guilabel:`Copy` to the right of the :guilabel:`Token`. This token will be be entered
into the :guilabel:`IoT Box Home Page`.
...
section by clicking on :guilabel:`Configure`. The, paste the *token* into the :guilabel:`Server
